...
const Print = ({ route }) => {
  ...
  useEffect(() => {
    const fetchAmount = async () => {
      try {
        const response = await axios.post(`${baseUrl}/payment/get-total-amount`, {
      ...
      <body>
        <div class="receipt">
          ...
          <div style="display: flex; flex-direction: row; justify-content: space-between; align-items: center; margin-bottom: 10px;">
            <p style="margin: 0;">Group: ${payInfo?.group_id?.group_name || "N/A"}</p>
            <p style="margin: 0;">Ticket: ${payInfo?.ticket || "N/A"}</p>
          </div>
          <table style="border-collapse: collapse; width: 100%;" border="1">
            <tr>
              <td style="padding: 5px; font-size:14px">Received Amount</td>
              <td style="padding: 5px; font-size:14px">Rs.${payInfo?.amount}</td>
            </tr>
          </table>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
      <View style={{ marginHorizontal: 22, marginTop: 12 }}>
        ...
        <View style={{ padding: 8, backgroundColor: "#f0eeee", borderRadius: 8, marginTop: 5 }}>
          <Text style={{ fontWeight: "bold", marginBottom: 0, fontSize: 18, textAlign: "center" }}>MY CHITS</Text>
          ...
          <View style={{ borderBottomWidth: 1, borderColor: "#000", marginVertical: 5 }} />

          <Text style={{ fontWeight: "bold", marginBottom: 0, fontSize: 13, textAlign: "center", marginBottom: 10 }}>Receipt</Text>
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
            ...
            <Text style={[styles.textStyle, { fontSize: 14, fontWeight: "bold" }]}>Ticket: {payInfo?.ticket || ""}</Text>
          </View>

          <Text style={styles.textStyle}> </Text>

          <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
            <Text style={[styles.textStyle, { fontSize: 16, fontWeight: "bold", borderWidth: 1, padding: 5 }]}>
              Received Amount | Rs.{payInfo?.amount || ""}
            </Text>
          </View>
          ...
          <Text style={styles.textStyle}>
            Mode: {payInfo?.pay_type ? payInfo.pay_type.charAt(0).toUpperCase() + payInfo.pay_type.slice(1) : ""}
          </Text>
